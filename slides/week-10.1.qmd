...
mod_fit <- lm(lib_dem ~ log_wealth, data = modelData)
```
...
a <- round(coef(mod_fit)[1], 2)
b <- round(coef(mod_fit)[2], 2)

fit_aug <- data.frame(
  log_wealth = mod_fit$model$log_wealth,
  lib_dem    = mod_fit$model$lib_dem,
  .fitted    = fitted(mod_fit),
  .resid     = residuals(mod_fit)
) |>
