...
lm(libdem ~ polarization, data = modelData) |> summary()
```
<br>
...
lm(libdem ~ polarization + log_wealth, data = modelData) |> summary()
```
...
lm(libdem ~ polarization + log_wealth + oil_rents, data = modelData) |> summary()
```
...
| $b_3$ is the impact of a 1-unit change in oil revenues per capita on the predicted level of Y, **holding polarization and GDP fixed** (all else equal) — note the negative sign: higher oil rents are associated with *lower* democracy, controlling for wealth
...
```{r}
lm(corruption ~ libdem + polarization, data = modelData) |> summary()
```
...
```{r}
lm(corruption ~ libdem + polarization + log_wealth, data = modelData) |> summary()
```
...
lm(libdem ~ factor(judicial_review), data = modelData) |> summary()
```
...
- **Slope:** countries with judicial review are expected, on average, to be 0.32 units more democratic on the liberal democracy index
...
- Average democracy score of countries with judicial review is 0.13 + 0.32 = 0.45
...
lm(libdem ~ region, data = modelData) |> summary()
```
...
lm(corruption ~ regime, data = modelData) |> summary()
```
