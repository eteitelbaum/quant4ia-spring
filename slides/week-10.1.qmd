...
lm(libdem ~ polarization, data = modelData) |> tidy()
```
<br>
...
lm(libdem ~ polarization + log_wealth, data = modelData) |> tidy()
```
...
lm(libdem ~ polarization + log_wealth + oil_rents, data = modelData) |> tidy()
```
...
```{r}
lm(corruption ~ libdem + polarization, data = modelData) |> tidy()
```
...
```{r}
lm(corruption ~ libdem + polarization + log_wealth, data = modelData) |> tidy()
```
...
lm(libdem ~ factor(judicial_review), data = modelData) |> tidy()
```
...
lm(libdem ~ region, data = modelData) |> tidy()
```
...
lm(libdem ~ newReg, data = modelData) |> tidy()
```
...
lm(corruption ~ regime, data = modelData) |> tidy()
```
